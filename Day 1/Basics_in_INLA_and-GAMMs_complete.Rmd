---
title: "Public Health with INLA - Day 1"
...
```
Baseline patient - Patient with average age, with a tumor thickness equal to the average thickness and who had the removal surgery in the earliest year, have sex = 0 and ulcer = 0.
...
```{r Example 2.d}
result1_sids <- inla(formula = SID74 ~ 1 + nwp_hat74,
                     family = "poisson",
                     E = E74,
                     data = nc.sids,
                     control.predictor = list(compute = TRUE))
...
```{r Example 2.g}
result2_sids <- inla(formula = SID74 ~ 1 + nwp_hat74 + 
                       f(CNTY.ID, model = "iid", hyper = list(theta = list(param = c(2,2), prior = "loggamma"))),
                     family = "poisson",
                     E = E74,
                     data = nc.sids,
                     control.predictor = list(compute = TRUE))
...
result2d_sids <- inla(formula = SID74 ~ 1 + nwp_hat74 + 
                       f(CNTY.ID, model = "iid"),
                     family = "poisson",
                     E = E74,
                     data = nc.sids,
                     control.predictor = list(compute = TRUE))
res2d_sd_rint <- inla.tmarginal(function(x) sqrt(1/x), result2d_sids$marginals.hyperpar$`Precision for CNTY.ID`)
plot(res2d_sd_rint, type = "l", xlab = expression(sigma), ylab = expression(paste("f(", sigma ,"|y)")))
```
```{r, investigate_prior}
#Default prior for tau in the iid model
plot(seq(0, 10, by=0.1),
     dgamma(seq(0, 10, by=0.1),
            shape = 1, scale = 0.0005),
     type = "l")
#Very strict
