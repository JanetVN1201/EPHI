---
title: "Public health with INLA - Day 3"
author: "Janet van Niekerk (janet.vanNiekerk@kaust.edu.sa/janet.vanNiekerk@up.ac.za)"
output: html_document
date: "October 2025"
---

```{r setup, include=FALSE}
knitr::opts_knit$set(root.dir = "/Users/vanniej/Documents/GitHub/EPHI")
...
hist(nc.sids$SID74)
summary(nc.sids)
nc.sids <- st_read(system.file("shapes/sids.gpkg", package = "spData")[1], quiet = T)
...
plot(nc.sids$geom)
plot(nc.adj, coords = nc.sids$geom, add = TRUE, col = "blue", cex=0.1, pch = 1)

#Add covariate to spatial dataset    
nc.sids$CNTY_ID2 <- 1:(length(nc.sids$CNTY_ID))

#Fit the spatial model
result3_sids <- inla(SID74 ~ 1 + nwp_hat74 + f(CNTY_ID2,
                                               model = "besagproper", graph = B.nc),
                     family = "poisson",
                     E = E74,
...
#Plot the u - plot the random spatial field
nc.sids$u_besag <- result3_sids$summary.random$CNTY_ID2$mean

plot(nc.sids["u_besag"], main = "Spatial field u")
...
```{r eval=FALSE, echo = TRUE}
inla.doc("bym") #BESAG + IID
inla.doc("bym2") #BYM but with interpretation (phi)
```
Let's revisit the NC SIDS dataset example.  
Now we add a BYM effect for the different counties,
```{r Example 2.a}
result4_sids <- inla(SID74 ~ 1 + nwp_hat74 + f(CNTY_ID2, model = "bym2", graph = B.nc),
                     family = "poisson",
                     E = E74,
                     control.predictor = list(compute = T),
                     data = as.data.frame(nc.sids),
...
```
We can visualise the spatial and specific (iid) effects as well. Tricky, because the first half of `r summary.random$CNTY_ID2` is the sum of besag and iid (bym2), and the second half is the besag part only.
```{r Example 2.e}
...
```{r pre-work, eval = F}
# read data 
merg.data <- readRDS("~/Documents/GitHub/EPHI/Day 3/MAP_data.RDS")
...
# Neighbourhood graph
nb <- poly2nb(map_2_c)
nb2INLA("~/Documents/GitHub/EPHI/Day 3/map.adj",nb = nb)
g <- inla.read.graph(filename = "~/Documents/GitHub/EPHI/Day 3/map.adj")
...
nb2INLA("~/Documents/GitHub/EPHI/Day 3/map1.adj",nb = nb1)
g1_alt <- inla.read.graph(filename = "~/Documents/GitHub/EPHI/Day 3/map1.adj")
...
formula = yy ~ -1 + m + offset(log(E1))+ offset(log(E2))+
  ...
  f(ID_copy, copy="ID", hyper = list(beta = list(fixed = F)))+
