---
title: "Public Health with INLA - Day 2"
...
```
## Example 1 - Temporal and spline models
...
airp.ar1 <- inla(formula = airp ~ 0 + year + f(ID, model = "ar1"),
                 family = "lognormal",
                 data = airp.data,
                 control.predictor = list(compute = T),
                 control.family = list(hyper = list(prec = list(param = c(1,0.2)))))
#Change the prior to do a sensitivity study

summary(airp.ar1)
plot(log(airp.data$airp), type = "l", xlab = "Year", ylab = "Log of the count of passengers")
lines(airp.ar1$summary.fitted.values$mean , col = "blue")
```
Next we try a seasonal model,
```{r seasonal}
#inla.doc("seasonal")

airp.seasonal <- inla(formula = airp ~ 0 + year + f(ID, model = "seasonal", season.length = 12),
                 family = "lognormal",
                 data = airp.data,
                 control.predictor = list(compute = T),
                 control.family = list(hyper = list(prec = list(param = c(1,0.00005)))))
...
```
Next we try the random walk models (stochastic spline),
```{r rw1}
#RW1 = AR1 with rho=1
#RW2 uses two other points to smooth
#inla.doc("rw")
airp.rw1 <- inla(formula = airp ~ 0 + year + f(ID, model = "rw1"),
                 family = "lognormal",
                 data = airp.data,
                 control.predictor = list(compute = T),
                 control.family = list(hyper = list(prec = list(param = c(1,0.00005)))))
...
airp.rw2 <- inla(formula = airp ~ 0 + year + f(ID, model = "rw2", scale.model = T),
                 family = "lognormal",
                 data = airp.data,
                 control.predictor = list(compute = T),
                 control.family = list(hyper = list(prec = list(param = c(1,0.00005)))))
...
plot(log(airp.data$airp), type = "l", xlab = "Year", ylab = "Log of the count of passengers
     ")
lines(airp.rw1$summary.fitted.values$mean , col = "purple", lty =3, lwd = 2)
...
##Random walk models for nonlinear effects
```{r}
x <- seq(-10,10, by = 0.01)
y = x^2
...
## Example 2 - Binary regression (revisited)
...
```{r Example 1.d}
#INLA model
model3.1 <- inla(formula = status ~ 1 + age + sex + year + f(thickness, model= "rw1") + ulcer,
                 family = "binomial",
                 data = melanoma)
```
...
## Example 3 - GLZ with random effects
